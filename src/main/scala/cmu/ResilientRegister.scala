...
class ResilientRegisters extends Transform with ResolvedAnnotationPaths {
  ...
  def execute(state: CircuitState): CircuitState = {
    val resilientRegTargets = state.annotations.collect { // Filters out everything except RRAnnotation
      case ResilientRegisterAnnotation(target) =>
        require(target.isLocal)
        target
    }

    val resilientRegsByModule = resilientRegTargets.groupBy(_.module) // creates a map from list
