...
//      if (portName.size == 2) {
//        println(s"HOHOHOHOHOHOHOHOHOOOO ${SubField(WRef(portName(0)), s"halt_${portName(1)}")}")
...
//        println(s"Haltconn - inst ${s.name}, ${halt_connects}")
...
//        val clk_conn_orig = Connect(info, Reference(lhs_name, ClockType, PortKind, lhs_flow), Reference(rhs_name, rhs_type, rhs_kind, rhs_flow))
//        val halt_conn = Connect(info, WRef(s"halt_${lhs_name}"), getHaltPortOf(rhs_name, inst2clk))
//        println(s"Haltconn - lhs_name: ${lhs_name}, rhs_name: ${rhs_name}, ${halt_conn}")
//        Block(Seq(clk_conn_orig, halt_conn))
//
//      case Connect(info, Reference(lhs_name, ClockType, PortKind, lhs_flow), SubField(rhs_ref, rhs_name, rhs_kind, rhs_flow)) =>
//        val clk_conn_orig = Connect(info, Reference(lhs_name, ClockType, PortKind, lhs_flow), SubField(rhs_ref, rhs_name, rhs_kind, rhs_flow))
//        val halt_conn = Connect(info, WRef(s"halt_${lhs_name}"), getHaltPortOf(s"${rhs_ref.serialize}/$rhs_name", inst2clk))
//        println(s"Haltconn - lhs_name: ${lhs_name}, rhs_name: ${rhs_ref.serialize}/$rhs_name, ${halt_conn}")
//        Block(Seq(clk_conn_orig, halt_conn))
//
//      case Connect(info, SubField(lhs_ref, lhs_name, ClockType, lhs_flow), SubField(rhs_ref, rhs_name, rhs_kind, rhs_flow)) =>
//        val clk_conn_orig = Connect(info, SubField(lhs_ref, lhs_name, ClockType, lhs_flow), SubField(rhs_ref, rhs_name, rhs_kind, rhs_flow))
//        val halt_conn = Connect(info, getHaltPortOf(s"${lhs_ref.serialize}/$lhs_name", inst2clk), getHaltPortOf(s"${rhs_ref.serialize}/$rhs_name", inst2clk))
//        println(s"Haltconn - lhs_name: ${lhs_ref.serialize}/$lhs_name, rhs_name: ${rhs_ref.serialize}/$rhs_name, ${halt_conn}")
//        Block(Seq(clk_conn_orig, halt_conn))
//
//      case _ =>
...
//
//    println(s"${m.ports.size} -> ${newPorts.size}")
//
//    m match {
...
//    println(s"MMM ${m.name}")
//    m map populateInst2Clk(inst2clk)
//  }
//
//  def printAll(s: Statement): Statement = {
//    println(s"CGA: ${s.getClass}, ${s}")
//    s map printAll
//  }
//
//  def populateInst2Clk(inst2clk: Inst2Clk)(s: Statement) : Statement = {
//    s match {
//      case Connect(info, SubField(WRef(inst_name, _, _, _), lhs_name, ClockType, lhs_flow), WRef(rhs_name, ClockType, rhs_kind, rhs_flow)) =>
...
//        println(s"KSKSKSKS $s ${lhs_name} -> ${SubField(Reference(rhs_inst_name), rhs_name, ClockType)}")
...
//        DefNode(info, lhs_name, rhs) map findRef(signals, clocks)
...
//    println(s"0: ${circuit.modules.size}")
...
//
//    println(s"1: ${circuit.modules.size}")
//
//    var i = 0
...
//
//    println(mod2clk("CoherenceManagerWrapper"))
//
//    fp.close()
